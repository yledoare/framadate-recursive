    ...
    <form action="{if $admin}{poll_url id=$admin_poll_id admin=true}{else}{poll_url id=$poll_id}{/if}" method="POST" id="poll_form">
        ...
        <table class="results">
            ...
            <tr>
                <th role="presentation"></th>
                {foreach $slots as $id=>$slot}
		{if "now"|date_format:'%Y-%m-%d' == $slot->day|date_format:"%Y-%m-%d"}
                    <th colspan="{$slot->moments|count}" style="background-color: green;" class="bg-primary day today" id="D{$id}">{$slot->day|date_format_intl:DATE_FORMAT_DAY|html}</th>
		{else}
		...
		{/if}
            ...
            <tr>
                ...
                {foreach $slots as $slot}
                    {foreach $slot->moments as $id=>$moment}
		{if "now"|date_format:'%Y-%m-%d' == $slot->day|date_format:"%Y-%m-%d"}
                        <th colspan="1" style="background-color: green;" class="bg-info" id="H{$headersDCount}">{$moment|html}</th>
		{else}
                ...
                <tr>
                    ...
                    {foreach $slots as $slot}
                      {foreach $slot->moments as $moment}
                        ...
                        {if $choice=='2'}
		{if "now"|date_format:'%Y-%m-%d' == $slot->day|date_format:"%Y-%m-%d"}
                            <td class="bg-success text-success" style="background-color: yellow;"  headers="M{$headersM[$k]} D{$headersD[$k]} H{$k}"><i class="glyphicon glyphicon-ok"></i><span class="sr-only">{__('Generic', 'Yes')}</span></td>
		{else}
