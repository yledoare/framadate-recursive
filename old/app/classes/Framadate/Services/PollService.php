...
class PollService {
     ...
     * @return array
     */
    public function computeBestChoices2($votes, $poll, $name) {
        if (0 === count($votes)) {
        ...
        foreach ($votes as $vote) {
            ...
            foreach ($choices as $i => $choice) {
                if (!isset($result['y'][$i])) {
                    $result['inb'][$i] = 0;
                    $result['y'][$i] = 0;
	//$fp = fopen('/tmp/data2.txt', 'w');
        //fwrite($fp, "\n Votes : ".$results);
        //$results = print_r($poll, true);
        //fwrite($fp, "\n Poll : ".$results);
        //$results = print_r($vote, true);
        //fwrite($fp, "\n Vote : ".$results);
        //fwrite($fp, "\n name : ".$vote->name);
       	//fclose($fp);
                }
    ...
    private function checkVoteConstraints($choices, $poll_id, $slots_hash, $name, $vote_id = FALSE) {
        ...
        $this->checkMaxVotes2($choices, $poll, $poll_id,$name);
     ...
     * @throws ConcurrentVoteException
     */
    private function checkMaxVotes2($user_choice, $poll, $poll_id,$name) {
        $votes = $this->allVotesByPollId($poll_id);
        if (count($votes) <= 0) {
            return;
        }
	$best_choices = $this->computeBestChoices2($votes, $poll, $name);
